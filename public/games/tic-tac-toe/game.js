...
class Tic_Tac_Toe {
  ...
  terminate(status) {
    let resultMessage = document.getElementById("gameResults");
    this.active = false;
    ...
    if (status === "Draw") {
      ...
      resultMessage.innerHTML = `${status} is the Winner!`;
    }
  ...
  check() {
    checkDirections: for (let i = 0; i < 4; i++) {
      ...
      const nextMatrix = [
        //Used so that you can check the multiple rows and columns, however there are only the two individual diagnols
        { x: 1, y: 0 },
        { x: 0, y: 1 },
        { x: 0, y: 0 },
        { x: 0, y: 0 },
      ...
      if (i < 3) {
        origin = this.position[0][0];
      } else {
        origin = this.position[this.length - 1][0];
      }
      ...
      checkMultiples: for (let k = 0; k < this.length; k++) {
        ...
        if (start.status != 0) {
          checkSquares: for (let j = 1; j < this.length; j++) {
            let square = this.position[start.y + matrix.y * j][
              start.x + matrix.x * j
            ];
            if (square.status !== start.status) {
              if (i > 1) continue checkDirections;
              //Only need to check the diagnols a single time
    ...
    if (this.moves === this.length * this.length) {
      this.terminate("Draw");
      return false;
    }
  }
