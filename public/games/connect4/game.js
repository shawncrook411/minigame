...
class Connect4 {
  ...
  terminate(status) {
    ...
    if (status === "Red" || status === "Yellow") {
      ...
      resultMessage.innerHTML = `${status}`;
    }
    ...
    checkDirections: for (let direction = 0; direction < 4; direction++) {
      ...
      checkColumns: for (let column of this.position) {
        checkSquares: for (let square of column) {
          ...
          if (
            square.x > this.width - this.winSize &&
            (direction === 1 || direction === 2)
          )
            continue checkSquares;
          if (square.y > this.height - this.winSize && direction !== 1)
            continue checkSquares;

          if (square.status != 0) {
            checkWin: for (let length = 0; length < this.winSize; length++) {
              let next = this.position[square.x + matrix.x * length][
                square.y + matrix.y * length
              ];
